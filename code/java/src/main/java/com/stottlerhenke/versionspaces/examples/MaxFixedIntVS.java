...
/**
 * A version space that learns the largest fixed value it has been trained with.
 * 
 ...
 * @see MinFixedIntVS
 */
public class MaxFixedIntVS extends VS<Object, Integer> {
   private Integer _lowerBound = Integer.MIN_VALUE;

   private final Integer _upperBound = Integer.MAX_VALUE;
   ...
   hypotheses() {
      final Integer value = _lowerBound;
    ...
    */
   @Override
   public void train(final Object in, final Integer out) {
      _lowerBound = Math.min(_upperBound, Math.max(_lowerBound, out));
   }
