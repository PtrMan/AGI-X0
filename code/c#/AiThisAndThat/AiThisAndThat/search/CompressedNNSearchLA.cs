...
using AiThisAndThat.math.fft;
using AiThisAndThat.neural;
using AiThisAndThat.misc;
...
namespace AiThisAndThat {
    ...
    public interface IGenericClassicalNetworkLearningAlgorithm<NetworkType>
    where
        NetworkType : IForwardPropagatableNetwork, IWeightedNetwork, IHasResult<float>
    {
	    void learn(NetworkType network);
    }
     ...
     */
    public class CompressedNNSearchLA<NetworkType>
    :
        IGenericClassicalNetworkLearningAlgorithm<NetworkType>
    where
        NetworkType : IForwardPropagatableNetwork, IWeightedNetwork, IBiasedNetwork, IHasResult<float>
    {
        ...
        public void learn(NetworkType network) {
        ...
        void learnForAmplitudeAndPhase(
            ...
            Debug.Assert(network.numberOfWeights + network.numberOfBiases <= fftReal.Length);
            double[] fftRealSubarrDouble = new double[network.numberOfWeights + network.numberOfBiases];
            Array.Copy(fftReal, fftRealSubarrDouble, network.numberOfWeights + network.numberOfBiases);

            float[] fftRealSubarr = new float[network.numberOfWeights + network.numberOfBiases];
            convertToFloat(fftRealSubarrDouble, ref fftRealSubarr);
            ...
            */
            ...
            float[] fftRealSubarrForWeights, fftRealSubarrForBiases;
            fftRealSubarrForWeights = new float[network.numberOfWeights];
            fftRealSubarrForBiases = new float[network.numberOfBiases];
            Array.Copy(fftRealSubarr, fftRealSubarrForWeights, network.numberOfWeights);
            Array.Copy(fftRealSubarr, network.numberOfWeights, fftRealSubarrForBiases, 0, network.numberOfBiases);

            // scale bias
            float biasScaleFactor = 2.0f;
            apply(ref fftRealSubarrForBiases, v => v * biasScaleFactor);

            network.updateWeights(fftRealSubarrForWeights);
            network.updateBiases(fftRealSubarrForBiases);
            ...
            foreach (TrainingTuple<float> iTrainingTuple in trainingSet) {
                // forward
                network.forwardPropagate(iTrainingTuple.input);


                // calculate error of output
                float error = calcResultError(network.result, iTrainingTuple.output);
                sumOfError += error;
            }
            ...
            lock(context.bestNetworkCandidateMutex) {
                ...
                if (isNetworkBetter) {
                    Console.WriteLine("{0}, stride={1}, error={2}", string.Join(",", network.result), configStride, sumOfError);
        ...
        // TODO< move to array helper >
        delegate float ManipulatorDelegate(float v);

        static void apply(ref float[] arr, ManipulatorDelegate del) {
            for( int i = 0; i < arr.Length; i++ )   arr[i] = del(arr[i]);
        }



        private void convertToFloat(double[] fftRealSubarrDouble, ref float[] fftRealSubarr) {
            Debug.Assert(fftRealSubarr.Length == fftRealSubarrDouble.Length);
            for( int idx = 0; idx < fftRealSubarr.Length; idx++ )   fftRealSubarr[idx] = (float)fftRealSubarrDouble[idx];
        }
