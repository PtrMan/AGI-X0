        ...
        if( operation == EnumOperation.ADD_CONNECTION ) {
            ...
            for( int tryCounter = 0; tryCounter < NUMBER_OF_TRIES; tryCounter++ ) {
                ...
                if( sourceType == Neuroid.Helper.EdgeWeightTuple.NeuronAdress.EnumType.HIDDEN ) {
                    List<Integer> neuronIndices = getTwoDisjunctNumbers(random, chosenMutationGeneticExpression.neurons.length);
                    ...
                    destinationIndex = random.nextInt(chosenMutationGeneticExpression.neurons.length);
                }
            ...
            chosenMutationGeneticExpression.neurons[index].isEnabled = true;
        }
        else if( operation == EnumOperation.DISBALE_NEURON ) {
            final int index = random.nextInt(chosenMutationGeneticExpression.getEnabledNeurons());
            chosenMutationGeneticExpression.neurons[index].isEnabled = false;
        }
            ...
            final int neuronIndex = random.nextInt(chosenMutationGeneticExpression.neurons.length);

            chosenMutationGeneticExpression.neurons[neuronIndex].firingThreshold += (float)delta;
        }
            ...
            for( int neuronIndex : neuronIndices ) {
                chosenMutationGeneticExpression.neurons[neuronIndex].isEnabled = true;
            }
            ...
            chosenMutationGeneticExpression.connectionsWithWeights.addAll(connections);
        }
    }
