    ...
    private void populateAfterDescriptor(NeuroidNetworkDescriptor descriptor) {
...

        {
            for( final Neuroid.Helper.EdgeWeightTuple<Float> iterationEdge : descriptor.connections ) {
                ...
                if( iterationEdge.sourceAdress.type == Neuroid.Helper.EdgeWeightTuple.NeuronAdress.EnumType.INPUT ) {
                    sourceVertex = graphVerticesForInputNeurons[iterationEdge.sourceAdress.index];
                }
                else {
                    sourceVertex = graphVerticesForHiddenNeurons[iterationEdge.sourceAdress.index];
                }

                if( iterationEdge.destinationAdress.type == Neuroid.Helper.EdgeWeightTuple.NeuronAdress.EnumType.INPUT ) {
                    destinationVertex = graphVerticesForInputNeurons[iterationEdge.destinationAdress.index];
                }
                else {
                    destinationVertex = graphVerticesForHiddenNeurons[iterationEdge.destinationAdress.index];
                }
